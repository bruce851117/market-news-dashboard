...
def build_news_prompt(batch):
    ...
    prompt += "1 = 通常應刪除。\n\n"
    prompt += "輸出要求：\n"
    prompt += "1. headline 請簡化成30個中文字以內，必須使用繁體中文，且不要扭曲原意。\n"
    prompt += "2. summary 請留空字串，但欄位必須存在。\n"
    prompt += "3. tags 用英文或常用市場代號，例如 Fed、UST、CPI、Oil、ECB、China、A-share、AI。\n"
    prompt += "4. 如果新聞不重要，請不要輸出該則新聞。\n"
    prompt += "5. 請務必只輸出 JSON array，不要加解釋，不要 markdown。\n\n"
    prompt += "輸出格式範例：\n"
    prompt += "[{\"datetime\":\"2026-06-29 07:30\",\"category\":\"bond\",\"importance\":5,\"headline\":\"Fed釋出偏鷹訊號\",\"summary\":\"\",\"tags\":[\"Fed\",\"UST\"],\"source\":\"華爾街見聞\",\"url\":\"https://wallstreetcn.com/live/global\"}]\n\n"
    prompt += "以下是快訊資料：\n"
    prompt += batch_json
